...
public class ScrobblingCommand {
    public static void register(CommandDispatcher<QuiltClientCommandSource> dispatcher,
        ...
        dispatcher.register(
                literal("scrobbler-setup").executes(c -> ScrobblingCommand.setup(c.getSource(), LastFMScrobbler.TYPE)));
    }
    ...
    private static int setup(QuiltClientCommandSource source, ScrobblerType<?> typeOfScrobbler)
            throws CommandSyntaxException {
        source.sendFeedback(Component.translatable("kit_tunes.scrobbler.setup.waiting"));

        String url = typeOfScrobbler.setupURL();
        source.sendFeedback(Component.literal(url)
        ...
        awaitedScrobbler.whenComplete((scrobbler, error) -> {
            if (error != null) {
                onSetupError(source, KitTunesBaseException.convert(error));
                return;
            }
