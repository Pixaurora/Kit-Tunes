...
public class KitTunesToastImpl implements Toast {
    ...
    public KitTunesToastImpl(Font font, KitTunesToastData toastData) {
        ...
        List<MutableComponent> lines = toastData.messageLines().stream().map(KitTunesUIImpl::componentToMinecraftType)
                .toList();
    ...
    public Toast.Visibility render(GuiGraphics graphics, ToastComponent manager, long frameTime) {
        ...
        for (FormattedCharSequence line : this.bodyLines) {
            graphics.drawString(client.font, line, bodyTextStart.x(), currentLineY, ChatFormatting.BLACK.getColor(),
                    false);
    ...
    private void drawToastBackground(GuiGraphics graphics, Point offset) {
        ...
        for (ToastBackgroundTile tile : this.tiles) {
            ...
            graphics.blitSprite(this.background, texture.width(), texture.height(), textureOffset.x(),
                    textureOffset.y(), tilePos.x(), tilePos.y(), tileSize.width(), tileSize.height());
        }
    }
}
