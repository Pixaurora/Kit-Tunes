...
public class RectangularButton implements Widget {
    ...
    public static RectangularButton vanillaButton(Point pos, Component text, TextProcessor font, ClickEvent action) {
        return new RectangularButton(DEFAULT_UNHIGLIGHTED_TEXTURE, DEFAULT_HIGHLIGHTED_TEXTURE, pos, text, font,
                action);
    }

    @Override
    public void draw(WidgetHandle handle, GuiDisplay gui, Point mousePos) {
        GuiTexture background = this.isWithinBounds(mousePos) ? this.highlightedBackground
                : this.unhighlightedBackground;
