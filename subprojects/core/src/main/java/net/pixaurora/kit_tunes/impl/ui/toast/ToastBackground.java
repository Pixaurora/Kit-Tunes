...
public class ToastBackground {
	...
	public ToastBackground(ToastBackgroundAppearance appearance, Point iconPos, Point titlePos, Point linesStartPos, int maxLineLength, int rightPadding, int bottomPadding) {
	...
	public Pair<List<ToastBackgroundTile>, Size> tiles(Size textSize) {
		...
		for (int i = 0; i < columns.size(); i++) {
			...
			int repetitionCount = i == 1 ? centerSegmentCounts.x() : 1; // If in the middle, draw the middle part multiple times.
		...
		Size toastSize = Size.of(pos.x(), pos.y()); // After iterating through the columns, the offset from the start = the size.
