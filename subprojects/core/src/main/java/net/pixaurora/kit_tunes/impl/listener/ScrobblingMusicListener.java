...
public class ScrobblingMusicListener implements MusicEventListener {
    ...
    public void onTrackStart(Track track) {
        KitTunes.SCROBBLER_CACHE
                .execute(scrobblers -> scrobblers.startScrobbling(new ScrobbledTrack(track, Instant.now())));
    }

    @Override
    public void onTrackEnd(Track track, ListeningProgress progress) {
        boolean longEnoughToScrobble = progress.amountPlayed().compareTo(Duration.ofMinutes(1)) > 0; // We use 60
                                                                                                     // seconds as the
                                                                                                     // amount of time
                                                                                                     // required to
                                                                                                     // scrobble a song,
        ...
        if (longEnoughToScrobble) {
            KitTunes.SCROBBLER_CACHE.execute(
                    scrobblers -> scrobblers.completeScrobbling(new ScrobbledTrack(track, progress.startTime())));
        } else {
