...
public class TextFieldImpl extends EditBox implements TextField {
    private final TextFieldBackground<ResourceLocation> background;
    private final Style style;
    private final Size size;

    public TextFieldImpl(Font font, TextFieldBackground<GuiTexture> background, Component defaultText, int maxLength) {
        super(font, 0, 0, background.normal().size().width(), background.normal().size().height(),
    ...
    public void setPos(int x, int y) {
        this.setX(x);
        // why did I need an access widener for this specifically but not X? who knows!
        this.y = y;
    }

    // For the Mixin class associated with EditBox
    public TextFieldBackground<ResourceLocation> background() {
